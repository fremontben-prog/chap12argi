...
REFERENCE_YIELDS = {
    "cassava":              (13.67, 2.0),   # (t/ha attendu, tolérance t/ha)
    "potatoes":             (16.69, 2.5),
    "plantains and others": (11.05, 2.0),
    "sweet potatoes":       ( 7.94, 2.0),
    "yams":                 ( 6.19, 1.5),
    "rice":                 ( 4.68, 1.0),
    "sorghum":              ( 2.29, 0.8),
    "wheat":                ( 2.24, 0.8),
...
class TestIntegrationPredict:
    ...
    def test_predict_different_conditions_change_yield(self):
        ...
        payload_dry  = {"crop": "cassava", "rainfall_mm": 200.0,  "avg_temp": 18.0, "pesticides_tonnes": 1000.0}
        payload_wet  = {"crop": "cassava", "rainfall_mm": 3000.0, "avg_temp": 28.0, "pesticides_tonnes": 200000.0}
...
class TestIntegrationRecommend:
    ...
    @skip_if_no_models
    def test_recommend_best_crop_is_potatoes(self):
        """Dans les conditions de référence, potatoes doit être recommandé."""
        r = client.post("/recommend", json=REFERENCE_CONDITIONS)
        assert r.json()["best_crop"] == "potatoes", (
            f"Meilleure culture attendue : potatoes, "
    ...
    def test_recommend_cold_climate_favors_wheat(self):
        """Climat froid et sec → le blé doit figurer dans le top 3."""
